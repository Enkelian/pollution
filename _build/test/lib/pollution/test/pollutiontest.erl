...
test_addExistingStations(M) ->
  [
    ?_assertEqual({error, alreadyExists}, addStation(M, {"Turku", {123.3,12.3}})),
    ?_assertEqual({error, alreadyExists}, addStation(M, {"Helsinki", {123.3,12.3}}))
  ].
...
test_addInvalidValue(M) ->
  [
    ...
    ?_assertEqual({error, noSuchStation}, addValue(M, {3,222}, {{2020,3,30},{0,49,1}}, "PM10", 111))
  ].
